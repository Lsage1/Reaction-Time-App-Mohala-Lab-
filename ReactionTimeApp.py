...
class ReactionTimeApp:
    ...
    def save_data(self, keys_str, felt_correctly, rt_str):
        ...
        # Use the filename that was set when experiment started
        ...
        # Prepare reaction times and keypress timestamps (up to 3)
        rt1 = f'{self.reaction_times[0]:.3f}' if len(self.reaction_times) > 0 else ''
        rt2 = f'{self.reaction_times[1]:.3f}' if len(self.reaction_times) > 1 else ''
        rt3 = f'{self.reaction_times[2]:.3f}' if len(self.reaction_times) > 2 else ''

        # Format keypress absolute timestamps
        kp_time1 = datetime.fromtimestamp(self.keypress_timestamps[0]).strftime('%Y-%m-%d %H:%M:%S.%f') if len(
            self.keypress_timestamps) > 0 else ''
        kp_time2 = datetime.fromtimestamp(self.keypress_timestamps[1]).strftime('%Y-%m-%d %H:%M:%S.%f') if len(
            self.keypress_timestamps) > 1 else ''
        kp_time3 = datetime.fromtimestamp(self.keypress_timestamps[2]).strftime('%Y-%m-%d %H:%M:%S.%f') if len(
            self.keypress_timestamps) > 2 else ''
    ...
    def start_button_pushed(self):
        ...
        # Get next available CSV filename
